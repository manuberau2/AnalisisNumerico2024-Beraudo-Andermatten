...
{
    public partial class FormularioTrapecioMultiple : Form
    {
        public FormularioTrapecioMultiple()
        {
            InitializeComponent();
        }
        ...
        private void BtnCalcularArea_Click(object sender, EventArgs e)
        {
            ...
            if (string.IsNullOrEmpty(TextBoxSubintervalos.Text))
            {
                MessageBox.Show("Ingrese el valor de subintervalos", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
            ...
            IntegralTrapecios integralTrapecios = new IntegralTrapecios();
            double areaResultante = integralTrapecios.CalcularIntegralTrapecioMultiple(funcion, xi, xd, subintervalos);
